...
void	separate_pipe(t_parse **lst)
{
	...
	while (cur)
	{
		if (cur->type == DEFA)
		{
			...
			while (seg[i])
			{
				separate_pipe_loop(lst, &cur, seg, i);
				i++;
			}
			free(seg);
		}
...
void	separate_pipe_loop(t_parse **lst, t_parse **cur, char **seg, int i)
{
	t_parse	*new;

	if (seg[i][0] == '|')
		new = new_parse(seg[i], PIPE, NONE);
	else
...
char	**split_pipe(char *s)
{
	...
	while (blk < seg)
	{
		if (set_start_end_pipe(s, &i, &blk, &res))
			return (0);
		blk++;
	}
	res[blk] = 0;
